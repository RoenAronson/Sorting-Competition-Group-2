...
public class Group13 {
        ...
        private static class FracComparator implements Comparator<Data> {
                ...
                public int compare(Data s1, Data s2) {
                        ...
                        // cmp=(s1.bigNumerator.multiply(s2.denominator)).compareTo(s2.bigNumerator.multiply(s1.denominator));
                        // // Compare a/b to c/d by finding ad-bc
        ...
        private static class Data {
                ...
                public Data(String line) {
                        ...
                        // can arise
                        // to ensure mutual exclusion we could use nested if-else... but I think this is
                        // easier to read
                        //
                        ...
                        if (posSlash != -1) { // We found a slash so expression is either mixed or pure
                                if (posSpace != -1) { // We found a slash *and* a space
                                        ...
                                        whole = new BigInteger(exprLine.substring(0, posSpace));// Get everything before
                                                                                                // the space
                                        ...
                                        value = (new BigDecimal(0)).add((new BigDecimal(exprLine.substring(posSpace + 1, posSlash))).divide((new BigDecimal(exprLine.substring(posSlash + 1))), 
                                                                                                                     new MathContext(20, RoundingMode.HALF_UP)));
                                }
                                numerator = new BigInteger(exprLine.substring(posSpace + 1, posSlash));// Get everything
                                                                                                       // before the
                                                                                                       // slash and
                                                                                                       // after the
                                                                                                       // space (if any)
                                // denominator = new BigInteger(exprLine.substring(posSlash + 1)); // Get everything after
                                                                                                // the slash
                                // BigDecimal numDec = new BigDecimal(numerator);
                                // BigDecimal denomDec = new BigDecimal(denominator);
                                // BigDecimal wholeDec = new BigDecimal(whole);
                ...
                public static void print_test(String s1, String s2) {
                        ...
                        testItem = new Data(s1);
                        testItem2 = new Data(s2);
